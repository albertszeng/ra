...
class TileBag():
  ...
  def draw_tile(self, tile=None, log=True):
  ...
  def print_contents_of_bag(self):
    print("Bag Contents:")
    for i in range(len(self.bag)):
...
class PlayerState():
  def __init__(self,
               player_name,
               starting_sun,
               starting_points=gi.STARTING_PLAYER_POINTS):
  ...
  def remove_all_tiles_by_index(self, lst_of_indexes, log=True):
    for index in lst_of_indexes:
      if log:
...
class GameState():
  ...
  def clear_auction_tiles(self):
    self.auction_tiles = []

  # give tiles to a player
  def give_tiles_to_player(self, player_index, tile_list):
    self.player_states[player_index].add_tiles(tile_list)

  # remove a list of tiles from the current player
  def remove_single_tiles_from_current_player(self, tile_indexes, log=True):
  ...
  def remove_single_tiles_from_player(self,
                                      tile_indexes,
                                      player_index,
                                      log=True):
    self.player_states[player_index].remove_single_tiles_by_index(tile_indexes,
                                                                  log=log)

  # remove all tiles in the list of indexes from the current player
  def remove_all_tiles_by_index_from_current_player(self,
                                                    tile_indexes,
                                                    log=True):
  ...
  # remove all tiles in the list of indexes from the current player
  def remove_all_tiles_by_index_from_player(self,
                                            tile_indexes,
                                            player_index,
                                            log=True):
    self.player_states[player_index].remove_all_tiles_by_index(tile_indexes,
                                                               log=log)

  # set who won an auction and must now resolve disasters
  def set_auction_winning_player(self, winning_player):
    self.auction_winning_player = winning_player

  # remove the auction winning player
  def clear_auction_winning_player(self):
  ...
  def mark_player_passed(self, player_index):
  ...
  # mark a player's bid
  def add_auction_sun(self, player, sun):
  ...
  def is_auction_started(self):
    return self.auction_started

  def auction_was_forced(self):
    return self.auction_forced

  def is_game_ended(self):
    return self.game_ended

  def disasters_must_be_resolved(self):
  ...
  def get_num_auction_tiles(self):
    return len(self.auction_tiles)
  ...
  def get_num_mons_to_discard(self):
    return self.num_mons_to_discard

  def get_num_civs_to_discard(self):
  ...
  def print_player_state(self, player_index):
    self.player_states[player_index].print_player_state()

  def print_player_states(self, verbose=False):
    for state in self.player_states:
      state.print_player_state(verbose=verbose)
      print("")

  def print_auction_tiles(self):
    print("Auction Tiles:")
