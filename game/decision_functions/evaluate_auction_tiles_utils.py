...
def value_civs(
    # The number of distinct civ tiles that the player does not have.
    num_distinct_new_civs: int,
    # Number of distinct civ tiles the player currently has.
    num_distinct_current_civs: int,
    # Number of ra tiles left before the round ends.
    ra_tiles_left_in_round: int,
    ...
    """
    assert (
        0 <= num_distinct_new_civs and num_distinct_new_civs <= 5
    ), f"cannot evaluate {num_distinct_new_civs} new civs"
    assert (
        0 <= num_distinct_current_civs and num_distinct_current_civs <= 5
    ), f"there cannot be {num_distinct_current_civs} current civs"
    assert (
        0 <= ra_tiles_left_in_round
        and ra_tiles_left_in_round <= gi.NUM_RAS_PER_ROUND[5]
    ), f"there cannot be {ra_tiles_left_in_round} ra tiles left"
    ...
    def second_civ_value(ra_tiles_left_in_round: int, how_many_sun_left: int) -> float:
        ...
        ra_tile_modifier: float = max(ra_tiles_left_in_round / 10.0, 0.0)
        # The less suns the player has, the less valuable the 2nd civ is
        sun_modifier: float = max((how_many_sun_left - 1) / 4.0, 0.0)
    ...
    cumulative_current_civ_values = {0: 0, 1: 5, 2: 5, 3: 10, 4: 15, 5: 20}
    return (
        cumulative_new_civ_values[num_distinct_new_civs + num_distinct_current_civs]
        - cumulative_current_civ_values[num_distinct_current_civs]
    )
...
def value_niles_and_flood(
    ...
    nile_value = num_new_niles * (
        num_rounds_left_inc_this_one * 0.5
        + (0.5 if num_new_floods + num_current_floods > 0 else 0)
    )
...
def value_pharaohs(
    ...
    num_opponent_pharaohs: Mapping[
        str, Tuple[int, bool]
    ],  # player_name -> (num pharaohs, is active)
    num_rounds_left_inc_this_one: int,
) -> float:
    ...
    assert (
        num_rounds_left_inc_this_one > 0
    ), "Cannot evaluate pharaohs if no rounds left"
    ...
    for _player_name, (num_opp_pharaohs, is_active) in num_opponent_pharaohs.items():
        most_opposing_pharaohs_possible = max(
            most_opposing_pharaohs_possible,
            num_opp_pharaohs + (num_new_pharaohs if is_active else 0),
        )
        most_opposing_pharaohs_currently = max(
            most_opposing_pharaohs_currently, num_opp_pharaohs
        )
        least_opposing_pharaohs_currently = min(
            least_opposing_pharaohs_currently, num_opp_pharaohs
        )
    num_opp_players_less_pharaohs = len(
        [
            n
            for (n, _is_active) in num_opponent_pharaohs.values()
            if n < num_current_pharaohs
        ]
    )
    at_risk_of_becoming_last = (
        num_current_pharaohs > least_opposing_pharaohs_currently
        and num_opp_players_less_pharaohs == 1
        and least_opposing_pharaohs_currently + num_new_pharaohs >= num_current_pharaohs
    )
    ...
    # if player is winning, but can be overtaken, value pharaohs highly
        ...
        num_current_pharaohs <= most_opposing_pharaohs_possible
        and num_current_pharaohs >= most_opposing_pharaohs_currently
    ):
    ...
    # if player can tie for the lead in pharaohs,
    # value pharaohs highy but a bit less than above
    elif num_current_pharaohs + num_new_pharaohs == most_opposing_pharaohs_currently:
        return num_new_pharaohs * (2.5 + 1.5 * (num_rounds_left_inc_this_one - 1))

    # if player cannot reach the pharaoh leader, and is not at risk of being last,
    # value pharaohs lightly
    elif (
        num_current_pharaohs + num_new_pharaohs < most_opposing_pharaohs_currently
        and not at_risk_of_becoming_last
    ):
        return num_new_pharaohs * num_rounds_left_inc_this_one

    # if player is at risk of being last or is last current but can overtake last,
    # value pharaohs moderately
    elif at_risk_of_becoming_last or (
        num_current_pharaohs + num_new_pharaohs > least_opposing_pharaohs_currently
        and num_current_pharaohs <= least_opposing_pharaohs_currently
    ):
        return 2 + (num_rounds_left_inc_this_one - 1)

    # the only remaining case should be if the player cannot catch up to 2nd to last.
    # In this case, value pharoahs lightly.
    else:
        return num_new_pharaohs * num_rounds_left_inc_this_one
