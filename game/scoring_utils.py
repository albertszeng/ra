...
def base_round_scoring(
        player_states: Iterable[gs.PlayerState]) -> None:
...
def calculate_unrealized_points(
    ...
    if is_final_round:
        round_end_points = calculate_round_end_points_gained(
            player_states)
        game_end_points = calculate_game_end_points_gained(
            player_states)
...
def calculate_value_of_auction_tiles(
    ...
    def_p_gained = {
        p_state.get_player_name(): 0 for p_state in p_states}
    def_end_p_gained = calculate_round_end_points_gained(
        p_states)
    default_game_end_points_gained = calculate_game_end_points_gained(
        p_states)
