...
CIV_ASTR = TileTypeInfo("Civilization -- Astronomy", 5, False,
                        TileType.COLLECTIBLE)
CIV_AGR = TileTypeInfo("Civilization -- Agriculture", 5, False,
                       TileType.COLLECTIBLE)
CIV_WRI = TileTypeInfo("Civilization -- Writing", 5, False,
                       TileType.COLLECTIBLE)
CIV_REL = TileTypeInfo("Civilization -- Religion", 5, False,
                       TileType.COLLECTIBLE)
...
MON_STE = TileTypeInfo("Monument -- Step Pyramid", 5, True,
                       TileType.COLLECTIBLE)
...
TILE_INFO = [
    GOD, GOLD, PHAR, NILE, FLOOD, CIV_ASTR, CIV_AGR, CIV_WRI, CIV_REL, CIV_ART,
    MON_FORT, MON_OBEL, MON_PAL, MON_PYR, MON_TEM, MON_STAT, MON_STE, MON_SPH,
    DIS_PHAR, DIS_NILE, DIS_CIV, DIS_MON, RA
]
...
def get_monuments_from_collection(collection):
  return collection[STARTING_INDEX_OF_MONUMENTS:(STARTING_INDEX_OF_MONUMENTS +
                                                 NUM_MONUMENTS)]
...
BID_NOTHING_OPTIONS = [
    "14", "b0", "b 0", "bid0", "bid 0", "bid nothing", "pass"
]
...
action_option_lst = [(DRAW, DRAW_OPTIONS, DRAW_DESC),
