...
class RaGame():
  ...
  def is_valid_num_players(self, num_players):
    return (num_players >= gi.MIN_NUM_PLAYERS
            and num_players <= gi.MAX_NUM_PLAYERS)
  ...
  def base_round_scoring(self, player_states):
    def least_and_most_num_pharoahs(player_states):
      ...
      for player_state in player_states:
        least_so_far = min(least_so_far,
                           player_state.collection[gi.INDEX_OF_PHAR])
        most_so_far = max(most_so_far,
                          player_state.collection[gi.INDEX_OF_PHAR])
    ...
    for player_state in player_states:
      # golden gods
      player_state.add_points(player_state.collection[gi.INDEX_OF_GOD] *
                              gi.POINTS_PER_GOD)

      # gold
      player_state.add_points(player_state.collection[gi.INDEX_OF_GOLD] *
                              gi.POINTS_PER_GOLD)
      ...
      if player_state.collection[gi.INDEX_OF_FLOOD] > 0:
        player_state.add_points(player_state.collection[gi.INDEX_OF_NILE] +
  ...
  def get_possible_actions(self):
    ...
    if self.game_state.is_auction_started():  # if it is an auction
      ...
      current_player_usable_sun = self.game_state.get_curr_player_usable_sun()
      ...
      if (self.game_state.get_current_player() !=
          ...
          or self.game_state.auction_was_forced()
          or self.game_state.get_num_auction_suns() > 0):
        legal_actions.append(gi.BID_NOTHING)

    else:  # if it is not an auction
      # if disaster must be resolved
      if (self.game_state.get_num_mons_to_discard() > 0
          or self.game_state.get_num_civs_to_discard() > 0):
        ...
        if self.game_state.get_num_civs_to_discard() > 0:
          possible_discards = [
              gi.DISCARD_ASTR, gi.DISCARD_AGR, gi.DISCARD_WRI, gi.DISCARD_REL,
              gi.DISCARD_ART
          ]
        ...
        # if there are monuments to be discarded
          ...
          possible_discards = [
              gi.DISCARD_FORT, gi.DISCARD_OBEL, gi.DISCARD_PAL, gi.DISCARD_PYR,
              gi.DISCARD_TEM, gi.DISCARD_STAT, gi.DISCARD_STE, gi.DISCARD_SPH
          ]
          for i in range(gi.NUM_MONUMENTS):  # the number of civilization tiles
            if winning_player_collection[gi.STARTING_INDEX_OF_MONUMENTS +
                                         i] > 0:
              legal_actions.append(possible_discards[i])
      ...
      # if no disaster to resolve
        ...
        if self.game_state.get_num_auction_tiles(
          ...
          if self.game_state.get_current_player_collection()[
              gi.INDEX_OF_GOD] > 0:
            possible_takes = [
                gi.GOD_1, gi.GOD_2, gi.GOD_3, gi.GOD_4, gi.GOD_5, gi.GOD_6,
                gi.GOD_7, gi.GOD_8
            ]
  ...
  def get_action(self, legal_actions, action_making_func=None, log=True):
    ...
    raise Exception(
        f"Unable to get legal action after {self.MAX_ACTION_ATTEMPTS} attempts"
    )
  ...
  def execute_action(self, action, legal_actions, tile_to_draw=None):
    ...
    def handle_auction_end():
      ...
      # if a sun was bid, give auction tiles to the winner
        ...
        self.game_state.exchange_sun(winning_player, max_sun,
        ...
        if num_niles_to_discard > 0:
          ...
          self.game_state.remove_single_tiles_from_player(
              ...
              [gi.INDEX_OF_NILE] * num_niles_to_discard, winning_player)
          ...
          if num_civs_owned <= num_civs_to_discard:
            self.game_state.remove_all_tiles_by_index_from_player(
                range(gi.STARTING_INDEX_OF_CIVS,
                      gi.STARTING_INDEX_OF_CIVS + gi.NUM_CIVS), winning_player)
          else:
        ...
        if num_mons_to_discard > 0:
          ...
          if num_mons_owned <= num_mons_to_discard:
            self.game_state.remove_all_tiles_by_index_from_player(
                range(gi.STARTING_INDEX_OF_MONUMENTS,
                      gi.STARTING_INDEX_OF_MONUMENTS + gi.NUM_MONS),
                winning_player)
          else:
    ...
    def execute_bid(n):
      sun_to_bid = self.game_state.get_curr_player_usable_sun()[n]
      self.game_state.add_auction_sun(self.game_state.get_current_player(),
                                      sun_to_bid)
    ...
    if action not in legal_actions:
      raise Exception(f"Cannot execute non-legal action '{action}'."
    ...
    if action == gi.DRAW:
      ...
      if tile == gi.INDEX_OF_RA:
        ...
        if self.game_state.get_num_ras_per_round(
          ...
          self.game_state.start_auction(True,
      ...
      was_forced = self.game_state.get_num_auction_tiles(
      ) == self.game_state.get_max_auction_tiles()
      self.game_state.start_auction(was_forced,
                                    self.game_state.get_current_player())
      self.game_state.advance_current_player()
    ...
    with open(self.outfile, "a+") as outfile:
      for action in action_lst:
        ...
        # if action is to draw
          ...
          t = self.execute_action(int(action[0]),
                                  legal_actions,
